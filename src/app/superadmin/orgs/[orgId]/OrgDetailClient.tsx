...
export default function OrgDetailClient({ org, employees: initialEmployees, invites: initialInvites }: Props) {
  ...
  const [tab, setTab] = useState<'employees' | 'invites'>('employees')
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-start justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-0.5">consultrack.vercel.app/{org.slug}</p>
          </div>
          <a
            href={`/${org.slug}`}
            target="_blank"
        ...
        <div className="flex gap-1 bg-slate-900 border border-slate-800 rounded-xl p-1 mb-6 w-fit">
          {(['employees', 'invites'] as const).map(t => (
            <button
              key={t}
              onClick={() => setTab(t)}
            ...
            >
              {t === 'employees' ? `Employees (${employees.length})` : `Invites (${invites.length})`}
            </button>
          ))}
        </div>
